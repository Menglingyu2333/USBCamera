...
//	}
  frameReceivePtr = &ov2640_framebuf1[0];
	return 0x00; 	//ok
}
//OV2640切换为JPEG模式
...
uint8_t ov2640_framebuf1[40*1024];				//帧缓存
uint8_t* ov2640_framebuf1_ptr = &ov2640_framebuf1[0];
uint8_t ov2640_framebuf2[40*1024];				//帧缓存
uint8_t* ov2640_framebuf2_ptr = &ov2640_framebuf2[0];
uint8_t* frameSendPtr;
uint8_t* frameReceivePtr;
uint8_t ImageBuf_1_Ready = 0;
uint8_t ImageBuf_2_Ready = 0;
uint32_t frameBuf_1_len = 0;
uint32_t frameBuf_2_len = 0;
uint32_t frameSend_len = 0;
uint32_t frameReceived_len = 0;
...
//	SCCB_WR_Reg(0XD3,60);
//	SCCB_WR_Reg(0XFF,0X01);
//	SCCB_WR_Reg(0X11,0X0);

	SCCB_WR_Reg(0XFF,0X00);
	SCCB_WR_Reg(0XD3,15);
	SCCB_WR_Reg(0XFF,0X01);
	SCCB_WR_Reg(0X11,0X02);
...
//  {
    ...
    while(OV2640_VSYNC==1)	//开始采集jpeg数据
    {
    	...
    	{
    		while(OV2640_PCLK==0);
    		ov2640_framebuf1[frameBuf_1_len]=OV2640_DATA;
    		while(OV2640_PCLK==1);
    		frameBuf_1_len++;
    	}
    }
    	pbuf=(uint8_t*)ov2640_framebuf1;
    	for(i=0;i<frameBuf_1_len;i++)//查找0XFF,0XD8
    	{
