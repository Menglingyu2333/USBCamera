...
void Test_Color(void)
{
	...
	Show_Str(lcddev.width-50,30,BLUE,YELLOW,"White",16,1);rt_thread_mdelay(500);
	LCD_Fill(0,0,lcddev.width,lcddev.height-0,BLACK);
	Show_Str(lcddev.width-50,30,BLUE,YELLOW,"Black",16,1);rt_thread_mdelay(500);
	LCD_Fill(0,0,lcddev.width,lcddev.height-0,RED);
	Show_Str(lcddev.width-50,30,BLUE,YELLOW,"Red",16,1); rt_thread_mdelay(500);
	LCD_Fill(0,0,lcddev.width,lcddev.height-0,GREEN);
	Show_Str(lcddev.width-50,30,BLUE,YELLOW,"Green",16,1);rt_thread_mdelay(500);
	LCD_Fill(0,0,lcddev.width,lcddev.height-0,BLUE);
	Show_Str(lcddev.width-50,30,WHITE,YELLOW,"Blue",16,1);rt_thread_mdelay(500);
...
//	TP_Adjust();  		//强制执行一次屏幕校准 (适用于没有IIC存储触摸参数的用户)
		...
		while(1)
	{
		...
		}else rt_thread_mdelay(10);	//没有按键按下的时候
