...
//    }
    ...
    if (jpegSendLen==0)
    {
        if(ImageBuf_1_Ready)
        {
          frameSendPtr = ov2640_framebuf1_ptr;
          frameSend_len = frameBuf_1_len;
        }
        else
        {
          frameSendPtr = ov2640_framebuf2_ptr;
          frameSend_len = frameBuf_2_len;
        }
        //每帧图像的起始包
        //初始化payload
        uvc_rdbuf[0] = 0x02;
        ...
        //
        bufCopy(frameSendPtr + jpegSendLen, payload, datalen);
        //从TX Buffer送到USB PMA Buffer
        UserToPMABufferCopy(uvc_rdbuf, ENDP1_BUF1Addr, PACKET_SIZE);
        jpegSendLen = datalen;

//        printf("a=%d b=%d c=%d  \n",jpegSendLen,frameSend_len,datalen);
//        printf("222=%x,jpegSendLen=%d\n",ov2640_framebuf1_ptr,jpegSendLen);
    }
    else
    {
        //单帧图像的后续包
        datalen = PACKET_SIZE - CAMERA_SIZ_STREAMHD;
            //判断是否最后一包
        if (jpegSendLen + datalen >= frameSend_len)
        {
            datalen = frameSend_len - jpegSendLen;
...
//        printf("111=%x,jpegSendLen=%d\n",ov2640_framebuf1_ptr + jpegSendLen,jpegSendLen);
...
//        datalen += CAMERA_SIZ_STREAMHD;
    ...
    if (jpegSendLen >= frameSend_len)
    {
      printf("j=%d k=%d \n",jpegSendLen,frameSend_len);
      jpegSendLen = 0;
